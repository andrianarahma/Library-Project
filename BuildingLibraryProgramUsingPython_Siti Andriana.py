...
def lihatData(listData) : 
    print(tabulate(listData, headers="keys",maxcolwidths=10,tablefmt="fancy_grid"))
...
#menarik data tertentu dari data induk
...
def buatListJudul(judul) :
    for i in range(len(semuaBuku)):
        if judul == semuaBuku[i]['Judul']:
            listJudul.append({
                'Label' : semuaBuku[i]['Label'],
                'Genre / Subgenre' : semuaBuku[i]['Genre']+" / "+semuaBuku[i]['Subgenre'],
                'Judul' : semuaBuku[i]['Judul'],
                'Penulis' : semuaBuku[i]['Penulis'],
                'ISBN' : semuaBuku[i]['ISBN'],
...
def tambahBuku() :
    ...
    for i in range(len(listBuku)):
        if isbnBuku == listBuku[i]['ISBN']:
            for j in range(listBuku[i]['Stok']+1,listBuku[i]['Stok']+stokBuku+1):
                label = listBuku[i]['Genre'][0]+listBuku[i]['Subgenre'][0]+"-"+listBuku[i]['Letak']+"-"+isbnBuku+"-"+str(j)
                semuaBuku.append({
                    'Genre' : listBuku[i]['Genre'],
                    'Subgenre' : listBuku[i]['Subgenre'],
                    'Judul' : listBuku[i]['Judul'],
                    'Penulis' : listBuku[i]['Penulis'],
                    'Letak' : listBuku[i]['Letak'],
                    'ISBN' : isbnBuku,
                    'Ketersediaan' : "Ada",
                    'Label' : label,
                    "Nama Peminjam" : "",
